...
public class AsyncTester
{
    ...
    public AsyncTester(ITypeRegistrar? registrar = null)
    {
        Registrar = registrar;
    }
        ...
        where T : Exception
    {
        ...
        try
        {
            RunAsync(args, console, c => c.PropagateExceptions());
    ...
    public CommandAppResult RunAsync(params string[] args)
    {
        var console = new TestConsole().Width(int.MaxValue);
        return RunAsync(args, console);
    }

    private CommandAppResult RunAsync(string[] args, TestConsole console, Action<IConfigurator>? config = null)
    {
        ...
        var result = app.RunAsync(args);

        int task = Convert.ToInt32(result.Result);
        ...
        return new CommandAppResult(task, output, context, settings);
    }
}