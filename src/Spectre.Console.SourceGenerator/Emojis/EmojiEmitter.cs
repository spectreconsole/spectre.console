...
internal static class EmojiEmitter
{
    ...
    public static string Emit(EquatableArray<EmojiModel> emojis)
    {
        ...
        using var writer = new IndentedTextWriter(stringWriter, "    ") { NewLine = "\n" };
        ...
        {
            writer.WriteLine("/// <summary>");
            writer.WriteLine($"/// Gets the \"{emoji.Description}\" emoji. {emoji.Emoji}");
            writer.WriteLine("/// </summary>");
        ...
        // Normalize to LF so the checked-in generated files are deterministic
        // regardless of the OS the generator runs on (the repo mandates LF).
